...
def get_ansible_check_k8s():
    source = commands.getoutput("""kubectl get deploy|grep nginx-dm""")
    output= source.split(' ')
    tu = [item for item in filter(lambda x:x != '', output)]
    if len(tu) > 0:
       kubectl_get_dict =  {"NAME":tu[0],"DESIRED":tu[1],"CURRENT":tu[2],"UP-TO-DATE":tu[3],"AVAILABLE":tu[4],"AGE":tu[5]}
