...
print_start_deploy_product "添加的minions节点kube-proxy安装"
...
inventory=${INVENTORY:-${INVENTORY_DIR}/inventory}
  ...
  ansible_playbook ${inventory} ${PLAYBOOKS_DIR}/add-minions-proxy.yml "$@"

echo "ansible_playbook  ${PLAYBOOKS_DIR}/add-minions-proxy.yml -i ${inventory}  "